from convenience.article_retrieval import *
import pandas as pd 
...
def main(): 
    excel_path = Path(__file__).parent / 'PCOS_Guideline_Dataset.xlsm'
    ...
    file_path = Path(__file__).parent / 'retrieval_results/api_retrieved.xlsx'
    ...
    with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
        ...
        input_df['id_sent_apiretrieval'] = input_df['included_article_pmid'].apply(lambda x: f'pmid:{x}' if not pd.isna(x) else x)
        ...
        for api, api_name in zip([oa_instance, ss_instance], ['oa', 'ss']):
            ...
            if 'included_article_id' in retrieved_ids.columns and 'included_article_id' in input_df.columns:
