...
def main():
    ...
    embase_search_dir = Path(__file__).parent / 'gdg_embase_searches/search_results/'
    embase_sent_df = pd.read_csv(Path(__file__).parent / 'gdg_embase_searches/embase_ids_sent.csv')
    ...
    for file in os.listdir(embase_search_dir):
        if file.endswith('.ris'):
            ...
            file_path = embase_search_dir / file
            try:
    ...
    file_path = Path(__file__).parent / 'retrieval_results/api_retrieved_embase.xlsx'
